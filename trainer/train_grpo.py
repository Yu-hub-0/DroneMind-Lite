...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DroneMind GRPO (Group Relative Policy Optimization)")
    ...
    parser.add_argument("--wandb_project", type=str, default="DroneMind-GRPO", help="wandb项目名")
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = f"DroneMind-GRPO-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
        wandb.init(project=args.wandb_project, name=wandb_run_name, id=wandb_id, resume=resume)
    
    # ========== 5. 初始化模型和数据 ==========
    base_weight = args.from_weight
    # Policy模型
    model, tokenizer = init_model(lm_config, base_weight, device=args.device, save_dir=args.save_dir)
    # Reference模型
    ref_model, _ = init_model(lm_config, base_weight, device=args.device, save_dir=args.save_dir)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if skip > 0: 
            ...
            grpo_train_epoch(epoch, loader, effective_iters, rollout_engine, ref_model, reward_model, start_step, wandb, use_sglang = (args.rollout_engine == "sglang"))
        else:
            effective_iters = min(len(loader), args.max_steps) if args.max_steps > 0 else len(loader)
            grpo_train_epoch(epoch, loader, effective_iters, rollout_engine, ref_model, reward_model, 0, wandb, use_sglang = (args.rollout_engine == "sglang"))
