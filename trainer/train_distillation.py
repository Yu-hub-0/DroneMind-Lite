...
from model.model_dronemind import DroneMindConfig
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DroneMind Knowledge Distillation")
    ...
    parser.add_argument("--wandb_project", type=str, default="DroneMind-Distillation", help="wandb项目名")
    ...
    lm_config_student = DroneMindConfig(hidden_size=args.student_hidden_size, num_hidden_layers=args.student_num_layers, use_moe=bool(args.student_use_moe))
    lm_config_teacher = DroneMindConfig(hidden_size=args.teacher_hidden_size, num_hidden_layers=args.teacher_num_layers, use_moe=bool(args.teacher_use_moe))
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = f"DroneMind-Distill-S{args.student_hidden_size}T{args.teacher_hidden_size}-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
        wandb.init(project=args.wandb_project, name=wandb_run_name, id=wandb_id, resume=resume)
    
    # ========== 5. 定义学生和教师模型 ==========
    model, tokenizer = init_model(lm_config_student, args.from_student_weight, device=args.device, save_dir=args.save_dir)
    Logger(f'学生模型总参数量：{sum(p.numel() for p in model.parameters()) / 1e6:.3f} M')
    teacher_model, _ = init_model(lm_config_teacher, args.from_teacher_weight, device=args.device, save_dir=args.save_dir)
    teacher_model.eval()
    teacher_model.requires_grad_(False)
