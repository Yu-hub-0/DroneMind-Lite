...
# 自定义的Critic模型，继承自DroneMindLM
class CriticModel(DroneMindForCausalLM):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DroneMind PPO (Proximal Policy Optimization)")
    ...
    parser.add_argument("--wandb_project", type=str, default="DroneMind-PPO", help="wandb项目名")
    ...
    lm_config = DroneMindConfig(hidden_size=args.hidden_size, num_hidden_layers=args.num_hidden_layers, use_moe=bool(args.use_moe))
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = f"DroneMind-PPO-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
        wandb.init(project=args.wandb_project, name=wandb_run_name, id=wandb_id, resume=resume)
    
    # ========== 5. 初始化模型和数据 ==========
    base_weight = args.from_weight
    # Actor模型
    actor_model, tokenizer = init_model(lm_config, base_weight, device=args.device, save_dir=args.save_dir)
