...
from trainer.trainer_utils import Logger, is_main_process, lm_checkpoint, init_distributed_mode, setup_seed, SkipBatchSampler, init_model, init_reward_model
from trainer.rollout_engine import create_rollout_engine, compute_per_token_logps
from scripts.drone_tools import execute_tool as execute_drone_tool
...
CHECK_ARGS = {
    ...
    "check_no_fly_zone": lambda a: a.get("latitude") is not None and a.get("longitude") is not None,
    "get_weather_for_flight": lambda a: bool(a.get("location")),
    "calculate_flight_time": lambda a: (
        a.get("battery_capacity_mah") is not None
        and a.get("battery_percentage") is not None
        and a.get("drone_weight_kg") is not None
    ),
    "plan_mission_route": lambda a: a.get("area_km2") is not None and a.get("flight_altitude_m") is not None,
    "lookup_drone_specs": lambda a: bool(a.get("model")),
    "get_drone_specs": lambda a: bool(a.get("model")),
    "convert_uav_units": lambda a: a.get("value") is not None and a.get("from_unit") and a.get("to_unit"),
    "convert_drone_units": lambda a: a.get("value") is not None and a.get("from_unit") and a.get("to_unit"),
}
...
def execute_tool(name, args):
    ...
    if not fn:
        try:
            return execute_drone_tool(name, args)
        except Exception:
            return None
    try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DroneMind Agent RL")
    ...
    parser.add_argument("--wandb_project", type=str, default="DroneMind-Agent-RL", help="wandb项目名称")
    ...
    parser.add_argument("--max_steps", type=int, default=0, help="Maximum training steps for quick validation; 0 means full epoch")
    ...
    lm_config = DroneMindConfig(hidden_size=args.hidden_size, num_hidden_layers=args.num_hidden_layers,
    ...
    reward_model = init_reward_model(args.reward_model_path, device=args.device, dtype=torch.float16)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if skip > 0:
            ...
            effective_iters = min(len(loader) + skip, args.max_steps) if args.max_steps > 0 else len(loader) + skip
            rl_train_epoch(epoch, loader, effective_iters, rollout_engine, ref_model, reward_model, start_step, wandb, use_sglang = (args.rollout_engine == "sglang"))
        else:
            effective_iters = min(len(loader), args.max_steps) if args.max_steps > 0 else len(loader)
            rl_train_epoch(epoch, loader, effective_iters, rollout_engine, ref_model, reward_model, 0, wandb, use_sglang = (args.rollout_engine == "sglang"))
