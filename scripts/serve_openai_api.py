...
def resolve_path(path, base_dir=PROJECT_ROOT):
...
def init_model(args):
    ...
    if os.path.basename(os.path.normpath(load_from)) == 'model':
        ...
        if args.lora_weight != 'None':
            apply_lora(model)
            load_lora(model, os.path.join(save_dir, 'lora', f'{args.lora_weight}_{args.hidden_size}.pth'))
    else:
        model = AutoModelForCausalLM.from_pretrained(load_from, trust_remote_code=True)
    print(f'DroneMind模型参数量: {sum(p.numel() for p in model.parameters()) / 1e6:.2f} M(illion)')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Server for DroneMind")
